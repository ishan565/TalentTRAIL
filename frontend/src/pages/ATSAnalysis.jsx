...
export default function ATSAnalysis() {
  ...
  useEffect(() => {
    api.activeResume().then((r) => setResumeId(r.id)).catch(() => setNoResume(true));
    // Load recommendations first; fall back to all stored jobs if none exist yet.
    api
      .recommendations()
      .then((recs) => {
        if (recs.length) {
  ...
  const run = async () => {
    ...
    setBusy(true);
    try {
      setResult(await api.atsScore(resumeId, jobId));
    } finally {
      setBusy(false);
    }
  };
